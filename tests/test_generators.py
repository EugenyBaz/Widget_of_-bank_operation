...
def test_filter_by_currency(filter_cur, currency, expected):
...
def test_filter_by_invalid_currency(filter_cur):
...
@pytest.mark.parametrize("transactions_discr, expected", [([
        {
            "id": 939719570,
            "state": "EXECUTED",
            "date": "2018-06-30T02:08:58.425572",
...
def test_transaction_descriptions(transactions_discr, expected):
    """Тест проверки на корректность работы вывода описания транзакции"""
    assert list(transaction_descriptions(transactions_discr))== expected
